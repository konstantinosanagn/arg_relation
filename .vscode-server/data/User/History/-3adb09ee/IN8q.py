...
def create_prompt(context, question, choices):
    return (
        ...
        f"{choices}\n"
        "You can analyze like this: [Thought Process]. So the answer is: [Answer].\n"
        f"The passage is: {context}\n"
        f"The question is: {question}\n"
        f"Here are 4 choices for it and they are:\n"
        f"{choices}\n"
        "[INST] Which one should I choose? Provide only the numbered choice (0 or 1 or 2 or 3) as the final answer. [/INST]"
    )

# Function to isolate the answer from the response
def isolate_answer(response):
    pattern = r"The passage is*Which one should I choose\? Provide only the number \(0 or 1 or 2 or 3\) as the final answer\.\s*(.*)"
