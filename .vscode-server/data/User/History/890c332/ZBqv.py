...
def create_prompt(row, response_parser):
    prompt = (
        ...
        f"1. {row['text']}\n"
        f"2. {row['sup_text']}\n"
        f"Does the second proposition support the first proposition?\nWith no other words, answer with 'yes' or 'no'.\n"
...
def predict_relations(df, response_parser):
    responses = generate_responses(df, response_parser)
    parsed_results = parse_responses(responses, response_parser)
    
    df['predicted_relation'] = parsed_results
    
    support_data = update_support_data(df, parsed_results)
