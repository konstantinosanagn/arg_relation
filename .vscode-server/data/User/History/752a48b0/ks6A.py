        ...
        for prop, surrounding in pairs:
            for s_prop in surrounding:
                dataset.append({
                    ...
                    'reasons': prop.reasons,  # Add reasons to dataset for evaluation
                    'evidence': prop.evidence  # Add evidence to dataset if needed
                })
...
def predict_relations(df, model, tokenizer, response_parser):
    ...
    # Reconstruct comments from the DataFrame for reasons_map creation
    comments_dict = {}
    for row in df.to_dict('records'):
        ...
        comments_dict[comment_id]['propositions'].append({
            'id': row['proposition_id'],
            'type': '',  # Type is not available in the DataFrame
            'text': row['text'],
            'reasons': row.get('reasons', []),  # Assuming reasons might be stored in the row
            'evidence': row.get('evidence', [])  # Assuming evidence might be stored in the row
        })
    
    # Deserialize the reconstructed comments
    comments = []
    for comment_dict in comments_dict.values():
        propositions = [
            ...
            ) for prop in comment_dict['propositions']
        ]
        comment = Comment(id=comment_dict['id'], propositions=propositions)
