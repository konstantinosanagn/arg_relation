...
def create_prompt(context, question, choices):
    return (
        ...
        f"{choices}\n"
        "Which one should I choose? Just answer with one of the letters A, B, C, or D without any explanation."
    )
...
def parse_response(response):
    ...
    # Find the first instance of a single letter from a-d, assuming it represents the answer
    match = re.search(r'\b([abcd])\b', isolated_response)
    if match:
        answer = match.group(1)
        return {'a': 0, 'b': 1, 'c': 2, 'd': 3}.get(answer, -1)
    return -1
